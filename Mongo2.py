...
window.configure(background="#F0F0F0")  # Set background color
...
def handle_collection_selection(event):
    ...
    if selected_collection == "crystallization_method":
        attributes = ["_id", "crystallizationMethodName", "crystallizationTemperatureK"]
    elif selected_collection == "experimental_technique":
        attributes = ["_id", "experimentalTechnique_Name"]
    elif selected_collection == "protein_structure":
        attributes = [
            "structureId",
            "classification",
            "macromoleculeType",
...
def create_attribute_labels(attributes):
    for i, attribute in enumerate(attributes):
        label = ttk.Label(frame, text=attribute + ":", background="#F0F0F0")  # Set label background color
...
def create_operation_buttons(collection_name):
    ...
    enter_button.configure(style="OperationButton.TButton")  # Apply custom button style
    ...
    update_button.configure(style="OperationButton.TButton")  # Apply custom button style
    ...
    delete_button.configure(style="OperationButton.TButton")  # Apply custom button style
    ...
    search_button.configure(style="OperationButton.TButton")  # Apply custom button style
...
def perform_search(collection_name):
    ...
    try:
        ...
        messagebox.showerror("Error", "Invalid query syntax. Please enter a valid JSON format. Make sure to enclose the entire query within double quotes (\"\"), including the attribute names and values.")
...
collection_label = ttk.Label(window, text="Select Collection:", background="#F0F0F0")  # Set label background color
collection_label.pack()

collection_dropdown = ttk.Combobox(window, values=["crystallization_method", "experimental_technique", "protein_structure"])
...
query_label = ttk.Label(window, text="Query:", background="#F0F0F0")  # Set label background color
query_label.pack()
...
# Create a button to clear the result text
clear_button = ttk.Button(window, text="Clear Results", command=clear_result_text)
clear_button.pack(pady=5)
clear_button.configure(style="OperationButton.TButton")  # Apply custom button style

# Create a custom style for the buttons
style = ttk.Style()
style.configure(
    "OperationButton.TButton",
    background="#007bff",  # Set button background color
    foreground="#FFFFFF",  # Set button text color
